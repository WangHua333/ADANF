...
class ADANFModel(BaseModel):
    def __init__(self, opt, step):
        super(ADANFModel, self).__init__(opt)
        self.opt = opt
        ...
        if weight_l1 and 1:
            missing_keys, unexpected_keys = self.netG.load_state_dict(torch.load(
                '../experiments/to_pretrain_netG/models/1000_G.pth'),
                strict=False)
    ...
    def optimize_parameters(self, step):
        ...
        if weight_l1 > 0:
            ...
            print("l1l1l1l1l1llllllllllllllllllllllllllllllllllllllllllllll")
            if flow_warm_up_iter is not None:
