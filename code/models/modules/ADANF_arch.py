...
import numpy as np
from models.modules.dat_arch import DAT
from models.modules.ConditionEncoder import ADAEncoder, NoEncoder
...
class ADANF(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        ...
        if opt['cond_encoder'] == 'ADAEncoder':
            self.RRDB = ADAEncoder(in_nc, out_nc, nf, nb, gc, scale, opt)
        else:
            print('WARNING: Cannot find the conditional encoder %s.' % opt['cond_encoder'])
